...
__global__ void bvh_scene_build_cuda(hittable_list **world_ptr, curandState *states, const int num,
                                     utils::NaiveVector<sphere> **sphere_vector) {
    ...
    for (int y = threadIdx.z + blockIdx.y * blockDim.z; y < height; y += blockDim.z * gridDim.y) {
        for (int x = threadIdx.y + blockIdx.x * blockDim.y; x < width; x += blockDim.y * gridDim.x) {
            ...
            for (int shfl_dist = threadPerPixel / 2; shfl_dist > 0; shfl_dist /= 2) {
                pixel_sampled_result.x += __shfl_xor_sync(utils::tId_to_warp_mask<threadPerPixel>(tid),
                                                          pixel_sampled_result.x, shfl_dist);
                pixel_sampled_result.y += __shfl_xor_sync(utils::tId_to_warp_mask<threadPerPixel>(tid),
                                                          pixel_sampled_result.y, shfl_dist);
                pixel_sampled_result.z += __shfl_xor_sync(utils::tId_to_warp_mask<threadPerPixel>(tid),
                                                          pixel_sampled_result.z, shfl_dist);
            }
            if (threadIdx.x == 0) {
...
__global__ void spheres_motion(utils::NaiveVector<sphere> **spheres_ptr, curandState *devStates) {
    ...
    for (auto sid = tid; sid < spheres->count; sid += gsize) {
        ...
        s = sphere{
            s.bounding_box().center() + random_in_cube(-5, 5, devStates + tid), s.bounding_box().x.size() / 2, 0
        };
    }
}
...
void render_scene_realtime_cuda(bvh_tree **scene, camera &cam, camera *cam_cuda, curandState *devStates,
    ...
    std::thread{
        [=, &render_finished, &cam, &scene, &builder, &spheres] {
...
void parse_arguments(int argc, char **argv, int &size, int &samples, int &depth, int &frame, int &obj_num,
                     int &parallel_build) {
...
int main(int argc, char *argv[]) {
    ...
    render_scene_realtime_cuda(builder.get_scene(), cam, camGpuPtr.cudaPtr, devStates.cudaPtr, frame, parallel_build,
                               builder, sphere_vector);
